...
(function () {
    ...
    (function (Util) { // Generic javascript utilities.
        ...
        (function (Type) {
            ...
            Type.text.match = function (t, o) {
                ...
                o = Gun.text.is(o) ? {
                    '=': o
                } : o || {}; // {'~', '=', '*', '<', '>', '+', '-', '?', '!'} // ignore uppercase, exactly equal, anything after, lexically larger, lexically lesser, added in, subtacted from, questionable fuzzy match, and ends with.
                if (Type.obj.has(o, '~')) {
                    t = t.toLowerCase()
                }
                if (Type.obj.has(o, '=')) {
    ...
    (function (Gun) { // Gun specific utilities.
        ...
        Gun.is.val = function (v) { // Valid values are a subset of JSON: null, binary, number (!Infinity), text, or a soul relation. Arrays need special algorithms to handle concurrency, so they are not supported directly. Use an extension that supports them if needed but research their problems first.
            ...
            if (Gun.bi.is(v) // by "binary" we mean boolean.
                || Gun.num.is(v) || Gun.text.is(v)) { // by "text" we mean strings.
        ...
        Gun.union = function (gun, prime, cb, opt) { // merge two graphs into the first.
            ...
            if (!ctx.graph) {
                ctx.err = {
                    err: Gun.log("No graph!")
                }
            }
            if (!prime) {
                ctx.err = {
                    err: Gun.log("No data to merge!")
                }
            }
            ...
            if (!Gun.is.graph(prime, null, function (val, field, node) {
                    ...
                    if (!Gun.num.is(Gun.is.node.state(node, field))) {
                        return ctx.err = {
                            err: Gun.log("No state on '" + field + "'!")
                        }
                    }
                }) || ctx.err) {
                return ctx.err = ctx.err || {
                    err: Gun.log("Invalid graph!", prime)
                }, ctx
            }
            ...
            (function union(graph, prime) {
                ...
                ctx.err = Gun.list.map(prime, function (node, soul) {
                    ...
                    if (!soul) {
                        return {
                            err: Gun.log("Soul missing or mismatching!")
                        }
                    }
                    ctx.count += 1;
    ...
    (function (Gun) { // Gun prototype chain methods.
        ...
        Gun.chain.opt = function (opt, stun) {
            opt = opt || {};
            var gun = this,
                root = (gun.__ && gun.__.gun) ? gun.__.gun : (gun._ = gun.__ = {
                    gun: gun
                }).gun.chain(); // if root does not exist, then create a root chain.
        ...
        Gun.chain.put = function (val, cb, opt) {
            ...
            function put(at) {
                ...
                var ctx = {
                    obj: val
                }; // prep the value for serialization
                ...
                if (Gun.is(val)) {
                    if (!ctx.field) {
                        return cb.call(chain, {
                            err: ctx.err = Gun.log('No field to link node to!')
                        }), chain._.at('err').emit(ctx.err)
                    }
                    return val.val(function (node) {
                        var soul = Gun.is.node.soul(node);
                        if (!soul) {
                            return cb.call(chain, {
                                err: ctx.err = Gun.log('Only a node can be linked! Not "' + node + '"!')
                            }), chain._.at('err').emit(ctx.err)
                        }
                ...
                if (ctx.field) {
                    ...
                    return cb.call(chain, ctx.err = {
                        err: Gun.log("No node exists to put " + (typeof val) + ' "' + val + '" in!')
                    }), chain._.at('err').emit(ctx.err)
                ...
                function end(err, ify) {
                    ...
                    if (ctx.field) {
                        return gun.back.path(ctx.field, null, {
                            chain: opt.chain || chain
                        });
                    }
                    if (ctx.not) {
                        return gun.__.gun.get(ctx.soul, null, {
                            chain: opt.chain || chain
                        });
                    }
                    chain.get(ctx.soul, null, {
                        chain: opt.chain || chain,
                        at: gun._.at
                    })
                }
                Gun.ify(ctx.obj, soul, {
                    pure: true
                })(end); // serialize the data!
            }
            if (gun === gun.back) { // if we are the root chain...
                put({
                    soul: Gun.is.node.soul(val) || Gun.text.random(),
                    not: true
                }); // then cause the new chain to save data!
        ...
        Gun.chain.get = (function () {
            ...
            return function (lex, cb, opt) { // get opens up a reference to a node and loads it.
                ...
                if (!Gun.obj.is(ctx.lex)) {
                    return ctx.cb.call(gun = gun.chain(), {
                        err: Gun.log('Invalid get request!', lex)
                    }), gun
                }
                if (!(ctx.soul = ctx.lex[Gun._.soul])) {
                    return ctx.cb.call(gun = this.chain(), {
                        err: Gun.log('No soul to get!')
                    }), gun
                ...
                function stream(err, data, info) {
                    ...
                    if (!data) {
                        ...
                        return ctx.by.chain._.at('null').emit({
                            soul: ctx.soul,
                            not: true
                        }).chain(ctx.opt.chain);
                    }
        ...
        Gun.chain.key = (function () {
            ...
            return function (key, cb, opt) {
                ...
                if (gun === gun.back) {
                    cb.call(gun, {
                        err: Gun.log('You have no context to `.key`', key, '!')
                    });
                } else {
        ...
        Gun.chain.path = (function () {
            ...
            return function (path, cb, opt) {
                ...
                if (gun === gun.back) {
                    cb.call(chain, opt.put ? null : {
                        err: Gun.log('You have no context to `.path`', path, '!')
                    });
                    return chain;
                }
        ...
        Gun.chain.map = function (cb, opt) {
            ...
            function each(val, field) {
                ...
                cb.hash[this.soul + field] = cb.hash[this.soul + field] || this.gun.path(field, path, {
                    chain: chain,
                    via: 'map'
                }); // TODO: path should reuse itself! We shouldn't have to do it ourselves.
                // TODO:
            ...
            gun.on(map, {
                raw: true,
                change: true
            }); // TODO: ALLOW USER TO DO map change false!
            if (gun === gun.back) {
        ...
        Gun.chain.not = function (cb, opt) {
            ...
            function not(at, e) {
                ...
                if (at.soul && gun.__.by(at.soul).node) {
                    return Gun.obj.del(at, 'not'), chain._.at(e).emit(at)
                }
                if (!at.not) {
                    return
        ...
        Gun.chain.set = function (item, cb, opt) {
            ...
            if (!Gun.is(item)) {
                return cb.call(gun, {
                    err: Gun.log('Set only supports node references currently!')
                }), gun
            ...
            item.val(function (node) { // TODO: BUG! Return proxy chain with back = list.
                ...
                if (!soul) {
                    return cb.call(gun, {
                        err: Gun.log('Only a node can be linked! Not "' + node + '"!')
                    })
                }
        ...
        Gun.chain.init = function (cb, opt) {
            ...
            gun._.at('null').event(function (at) {
                ...
                if (at.soul) {
                    ...
                    gun.__.gun.put(Gun.is.node.soul.ify({}, soul), null, {
                        init: true
                    });
    ...
    (function (Gun) { // Javascript to Gun Serializer.
        function ify(data, cb, opt) {
            ...
            if (!data) {
                return ctx.err = {
                    err: Gun.log('Serializer does not have correct parameters.')
                }, end
            }
            if (ctx.opt.start) {
        ...
        function map(ctx, cb) {
            ...
            Gun.obj.map(ctx.at.obj, function (val, field) {
                ...
                if (String(field).indexOf('.') != -1 || (false && notValidField(field))) { // TODO: BUG! Do later for ACID "consistency" guarantee.
                    return ctx.err = {
                        err: Gun.log("Invalid field name on '" + ctx.at.path.join('.') + "'!")
                    };
                }
                if (!Gun.is.val(val)) {
                    var at = {
                    ...
                    if (!Gun.obj.is(val)) {
                        return ctx.err = {
                            err: Gun.log("Invalid value at '" + at.path.join('.') + "'!")
                        };
                    }
                    if (was = seen(ctx, at)) {
